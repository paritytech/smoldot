...
/// Cold-startup smoke: spawn westend-local + people-westend-local from
/// committed DB snapshots, hand smoldot a chain spec carrying
/// `lightSyncState` (no persisted DB), assert it warp-syncs from the
/// checkpoint and sees new parachain blocks.
#[tokio::test(flavor = "multi_thread")]
async fn smoke_cold() -> Result<(), anyhow::Error> {
