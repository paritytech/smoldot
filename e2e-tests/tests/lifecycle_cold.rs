...
//! `lifecycle_unstable_follow` on a network restored from a snapshot, with smoldot starting
//! from the spec checkpoint: the relay chain warp syncs and must report `syncing` progress
//! before `ready`.
...
async fn lifecycle_cold() -> Result<(), anyhow::Error> {
