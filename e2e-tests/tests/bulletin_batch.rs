...
/// GCS URLs for the snapshots produced by `bulletin_generate_snapshot`. Same
/// artifacts as `bulletin_fetch.rs`.
...
fn print_dev_mode_invocation(env_pairs: &[(&str, &str)]) {
    ...
    println!("  node js/bulletin_batch.js");
    println!();
}
    ...
    let cfg = NetworkConfigBuilder::new()
        ...
        .with_parachain(|p| {
            p.with_id(bulletin::PARA_ID)
                ...
                .cumulus_based(true)
                // Skip the embedded relay client and proxy relay-chain
                // queries through alice/bob's RPC. Zombienet expands the
                // `{{ZOMBIE:<node>:ws_uri}}` templates at spawn time. This
                // sidesteps the relay-side libp2p discovery quirks we hit
                // with the embedded relay (see polkadot-sdk's
                // `full_node_warp_sync/common.rs` for the same pattern on
                // collators "four" / "five").
                .with_default_args(vec![
                    "--ipfs-server".into(),
                    "-lsub-libp2p::bitswap=trace".into(),
                    "-lsync=debug".into(),
