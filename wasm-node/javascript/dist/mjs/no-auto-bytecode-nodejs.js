...
function errorReason(error) {
    var _a;
    return ((_a = error.errors) === null || _a === void 0 ? void 0 : _a.map((cause) => cause.message).filter(Boolean).join("; "))
        || error.message
        || error.code
        || "Error";
}
...
function connect(config) {
    if (config.address.ty === "websocket") {
        ...
        socket.on('close', (hasError) => {
            if (resetCalled)
                return;
            config.onConnectionReset(lastError !== null && lastError !== void 0 ? lastError : (hasError ? "Error" : "Closed gracefully"));
        });
