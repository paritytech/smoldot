...
//!
//! The main purpose of this collection is to provide a convenient API that is fundamentally
//! compatible with back-pressure mechanisms.
//!
//! When it comes to incoming traffic, the [`Network::next_event`] must be polled regularly in
//! order to process the events generated by the connections. If these events aren't processed
//! quickly enough, calling [`Network::read_write`] will stop reading data incoming on the
//! connection and therefore back-pressure it.
//!
//! # Basic usage
//!
...
// TODO: text below needs small update
...
struct Guarded<TNow> {
    ...
    connections: slab::Slab<Arc<Mutex<Connection<TNow>>>>,
    ...
    TNow: Clone + Add<Duration, Output = TNow> + Sub<TNow, Output = Duration> + Ord,
{
    ...
    pub async fn insert(&self) -> ConnectionId {
        let mut guarded = self.guarded.lock().await;

        let connection_id = guarded.next_connection_id;
        guarded.next_connection_id.0 += 1;

        let connection_index = guarded.connections.insert(());
    ...
    TNow: Clone + Add<Duration, Output = TNow> + Sub<TNow, Output = Duration> + Ord,
{
    ...
    async fn propagate_pending_event<TPeer>(&mut self, guarded: &mut Guarded<TNow>) {
