...
struct Background {
    ...
    /// Stream of best blocks updates. The background task stops if this stream ends.
    blocks_stream: Pin<Box<dyn Stream<Item = Vec<u8>> + Send>>,
...
impl Background {
    /// Starts the background task that updates the [`LatestKnownRuntime`] until
