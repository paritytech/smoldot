...
const startInstance = async (config) => {
  ...
  state.forEach((message) => {
    ...
    result.instance.exports.json_rpc_send(ptr, len, message.chain_index, message.source_id);
  });
...
compat.setOnMessage((message) => {
  ...
  if (state == null) {
    ...
    state.exports.json_rpc_send(ptr, len, message.chain_index, message.source_id);
  }
});
