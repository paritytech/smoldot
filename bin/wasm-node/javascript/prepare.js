...
try {
    child_process.execSync(
        "wasm-opt -o autogen/tmp.wasm -Os --strip-debug --vacuum --dce ../../../target/wasm32-wasi/debug/smoldot_js.wasm",
        { 'stdio': 'inherit' }
    );
} catch(error) {
    console.warn("Failed to run `wasm-opt`. Using the direct Rust output instead.");
    console.warn(error);
    fs.copyFileSync("../../../target/wasm32-wasi/debug/smoldot_js.wasm", "./autogen/tmp.wasm");
}
