...
const tmpDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'smoldot-wasm-build-'));
try {
    const wasmOptOutput = path.join(tmpDir, 'tmp.wasm');

    let fallbackCopy = false;
    try {
        if (buildProfile == 'min-size-release') {
            child_process.execSync(
                "wasm-opt -o " + wasmOptOutput + " -Oz --strip-debug --vacuum --dce "
    ...
    if (fallbackCopy) {
        fs.copyFileSync("../../../target/wasm32-wasi/" + buildProfile + "/smoldot_light_wasm.wasm", wasmOptOutput);
    }
    ...
    // small size files.
    const wasmOptOut = fs.readFileSync(wasmOptOutput);
    let base64Data = zlib.deflateSync(wasmOptOut).toString('base64');
    let imports = '';
    let fileNum = 0;
    let chunksSum = '""';
