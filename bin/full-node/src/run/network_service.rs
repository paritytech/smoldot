            ...
            (config.tasks_executor)(Box::pin({
                ...
                let future = async move {
                    loop {
                        ...
                        let task = {
                            let mut lock = inner.guarded.lock().await;
                            let (connection_id, connection_task) = lock
                                .network
                                .add_incoming_connection(Instant::now(), multiaddr.clone());

                            let (tx, rx) = mpsc::channel(16); // TODO: ?!
                            lock.active_connections.insert(connection_id, tx);
...
async fn update_round(inner: &Arc<Inner>, event_senders: &mut [mpsc::Sender<Event>]) {
    ...
    'events_loop: loop {
        let event = loop {
            let inner_event = match lock.network.next_event(Instant::now()) {
                Some(ev) => ev,
                ...
                service::Event::KademliaDiscoveryResult {
                    ...
                    match result {
                        Ok(nodes) => {
                            ...
                            lock.network.discover(&Instant::now(), chain_index, nodes);
                        }
                        Err(error) => {
        ...
        lock = inner.guarded.lock().await;
    }

    // TODO: doc
    for chain_index in 0..lock.network.num_chains() {
        loop {
            let assigned_peer = lock.network.assign_slots(chain_index);
    ...
    loop {
        ...
        lock.num_pending_out_attempts += 1;
...
async fn opening_connection_task(
    ...
    let socket = match multiaddr_to_socket(&start_connect.multiaddr) {
        ...
        Err(_) => {
            ...
            let mut lock = inner.guarded.lock().await;
            lock.num_pending_out_attempts -= 1;
            lock.network.pending_outcome_err(start_connect.id, true);
    ...
    let socket = {
        ...
        futures::select! {
            _ = timeout => {
                let mut lock = inner.guarded.lock().await;
                lock.num_pending_out_attempts -= 1;
                lock.network.pending_outcome_err(start_connect.id, false);
                inner.wake_up_main_background_task.notify(1);
                return;
            }
            result = socket => {
    ...
    let mut lock = inner.guarded.lock().await;
    lock.num_pending_out_attempts -= 1;
    let (connection_id, connection_task) = lock.network.pending_outcome_ok(start_connect.id);
    inner.wake_up_main_background_task.notify(1);
