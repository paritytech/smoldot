...
$ echo 'subjectAltName = DNS:www.parity.tech localhost 127.0.0.1 ::1' > "${EXTFILE}"

$ SERVER_NAME='server'
$ openssl ecparam -name prime256v1 -genkey -noout -out "${SERVER_NAME}.pem"
$ openssl req -key "${SERVER_NAME}.pem" -new -sha256 -subj '/O=Parity/OU=Smoldot' -out "${SERVER_NAME}.csr"
